...
namespace HotelProject.WebUI.Controllers
{
    public class BookingAreaAdminController : Controller
    {
        ...
        public async Task<IActionResult>ApprovedReservation(ApprovedResarvationDto approvedResarvationDto)
        {
            approvedResarvationDto.Status = "Onaylandı";
            var client = _httpClientFactory.CreateClient(); 
            var jsonData = JsonConvert.SerializeObject(approvedResarvationDto);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:5062/api/Booking/aaa", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
